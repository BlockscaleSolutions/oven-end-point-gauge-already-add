...
contract('MiniDonationRegistry.claimDonation()', (accounts) => {
  ...
  describe('Sending a valid redemption transaction', () => {
    ...
    it('should emit the DonationClaimed event', async () => {
      const res = await donationRegistry.claimDonation(0, recipient, transferAmount);
  ...
  describe('Sending a valid redemption transaction with a to address', () => {
    it('should update from and to balances correctly', async () => {
      await donationRegistry.claimDonation('0x2', recipient, transferAmount);
      const from = (await donationRegistry.donationBalances_(recipient)).toNumber();
      const to = (await donationRegistry.donationBalances_('0x2')).toNumber();
