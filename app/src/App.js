...
async function initContract() {
  window.web3.version.getNetwork(async (err, netId) => {
    ...
    window.debt = await window.web3.eth.contract(DebtArtifacts.abi).at(address);
    initListeners(window.debt);

    console.log('send send send');
    console.log('send send send');
    console.log(await window.debt.createLoanRequest(100, '0x1', '0x2', { from: window.from, gas: 1e6 }));
  });
}
